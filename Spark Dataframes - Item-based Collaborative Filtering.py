""" Finding movie recommendations to a given movie through Cosine Similarity using Spark Dataframes """
...
# Function to compute the Cosine Similarity score of ratings for each movie pair
def computeCosineSimilarity(df):
    ...
    # Calculate the Cosine Similarity score for each movie pair using numerator and denominator,
    # and include relevant columns only in the output dataframe
    df_score = df_score.withColumn('score', get_score(F.col('numerator'), F.col('denominator'))).select(['movieId1', 'movieId2', 'score', 'numPairs'])
...
def printRecommendations(results, targetMovieId):
    for result in results:
        ...
        print('{} viewers also watched:\n{}\nSimilarity Score: {}\n' \
                .format(strength, recommendedMovieName, similarityScore))
...
def main():
    # Create spark object
    # .master('local[*]'): use every CPU core on the local machine to execute the job
    ...
    movieDF = spark.read.csv('./data/ml-100k/u.data', header = False, sep = '\t', schema = movieSchema)

    # Discard the rows where ratings are lower than 3,
    # and sort by userId and movieId in ascending order
    movieDF_filtered = movieDF.filter(movieDF.rating >= 3).sort(['userId', 'movieId'])

    # Include relevant columns only in ratingDF
    ratingDF = movieDF_filtered.select(['userId', 'movieId', 'rating'])
    # Create two copies of the ratingDF and rename their columns properly for later join
    ratingDF_1 = ratingDF.withColumnRenamed('movieId', 'movieId1').withColumnRenamed('rating', 'rating1')
    ratingDF_2 = ratingDF.withColumnRenamed('movieId', 'movieId2').withColumnRenamed('rating', 'rating2')

    # Join the two idential dataframes,
    # and filter to get unique pairs of movies watched by the same users
    joinedDF = ratingDF_1.join(ratingDF_2, how = 'inner', on = 'userId').where('movieId1 < movieId2')

    # Compute Consine Similarity score and number of shared viewers for each unique pair of movies
    ...
    if len(sys.argv) > 1:
        ...
        # Filter the dataframe to get the movie pairs for the target movie that exceed the specified thresholds above
        ...
        top10_scores = moviePairsScoredDF_filtered.sort(F.desc('score')).head(10)
        ...
        print('Movie ID not provided!\nPlease enter a valid movie ID and run the script again.')
